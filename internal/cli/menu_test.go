...
// 3 строки шапки + по 2 на шаг + пустая + строка подтверждения + пустая + подсказка
func wantLines(steps int) int { return 3 + steps*2 + 4 }
...
// Строка подтверждения это отдельная позиция курсора, а не шаг: на ней Enter
// запускает, а не переключает.
func TestRenderMenuShowsConfirmRow(t *testing.T) {
	all := provision.Steps()
	picked := make([]bool, len(all))
	picked[0] = true
	picked[1] = true

	// курсор на шагах: строка подтверждения показана, но не подсвечена
	out := captureStdout(t, func() { renderMenu(all, picked, &provision.State{}, 0) })
	if !strings.Contains(out, "Continue") && !strings.Contains(out, "Продолжить") {
		t.Fatal("строки подтверждения нет в меню")
	}
	if strings.Count(out, "❯") != 1 {
		t.Error("курсор должен быть ровно в одном месте")
	}

	// счётчик выбранного виден заранее, без пересчёта кружков глазами
	if !strings.Contains(out, "2") {
		t.Error("количество выбранных шагов не показано")
	}

	// курсор на строке подтверждения
	out = captureStdout(t, func() { renderMenu(all, picked, &provision.State{}, len(all)) })
	confirm := ""
	for _, l := range strings.Split(out, "\r\n") {
		if strings.Contains(l, "Continue") || strings.Contains(l, "Продолжить") {
			confirm = l
		}
	}
	if !strings.Contains(confirm, "❯") {
		t.Errorf("на последней позиции курсор должен стоять на подтверждении, строка: %q", confirm)
	}

	// ничего не выбрано это отдельная формулировка, а не "выбрано: 0"
	out = captureStdout(t, func() {
		renderMenu(all, make([]bool, len(all)), &provision.State{}, len(all))
	})
	if !strings.Contains(out, "nothing selected") && !strings.Contains(out, "ничего не выбрано") {
		t.Error("пустой выбор не назван словами")
	}
}
