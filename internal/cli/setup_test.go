...
// Системная локаль на язык не влияет: английский у всех, пока не попросили иначе.
func TestSystemLocaleDoesNotChangeLang(t *testing.T) {
	for _, key := range []string{"LANG", "LC_ALL", "LC_MESSAGES"} {
		t.Setenv(key, "ru_RU.UTF-8")
	}
	t.Setenv("DJAPLOY_LANG", "")

	if got := detectLang(); got != provision.LangEN {
		t.Errorf("при ru-локали системы получили %q, ожидался английский", got)
	}

	t.Setenv("DJAPLOY_LANG", "ru")
	if got := detectLang(); got != provision.LangRU {
		t.Errorf("DJAPLOY_LANG=ru дал %q", got)
	}
}
