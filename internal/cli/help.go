...
SETUP
  ...
  Running djaploy setup with no flags asks for everything: which steps
  to run, the deploy user name, and a final confirmation. The flags below are
  for scripts and for skipping the questions.
  ...
  --lang CODE      output language: en (default), ru
...
SETUP
  ...
  Команда djaploy setup без флагов спросит всё сама: какие шаги выполнить, как
  назвать deploy-пользователя, подтверждение в конце. Флаги ниже нужны для
  скриптов и чтобы пропустить вопросы.
  ...
  --lang CODE      язык вывода: en (по умолчанию), ru
