...
func arrowMenu(all []provision.Step, picked []bool, state *provision.State) ([]provision.Step, error) {
	...
	// Последняя позиция курсора это строка "продолжить", а не шаг. Так выбор и
	// подтверждение делаются одной клавишей, без отдельного пробела.
	total := len(all) + 1
	cursor := 0
	lines := 0
	...
	for {
		...
		case keySpace:
			// пробел тоже отмечает: привычка из других меню
			if cursor < len(all) {
				picked[cursor] = !picked[cursor]
			}
		case keyQuit:
...
func renderMenu(all []provision.Step, picked []bool, state *provision.State, cursor int) int {
	...
	// Сколько выбрано, видно прямо в строке подтверждения: иначе перед запуском
	// приходится пересчитывать кружки глазами.
	count := 0
	for _, ok := range picked {
		if ok {
			count++
		}
	}
	label := fmt.Sprintf(t("Continue (%d selected)", "Продолжить (выбрано: %d)"), count)
	if count == 0 {
		label = t("Continue (nothing selected)", "Продолжить (ничего не выбрано)")
	}
	if cursor == len(all) {
