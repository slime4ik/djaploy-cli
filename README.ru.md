...
Готовит Linux-сервер к запуску приложения в Docker: свежие пакеты, fail2ban,
Docker с зеркалами реестра и отдельный непривилегированный пользователь для
деплоя.

**Без аккаунта. Без регистрации. Ничего никуда не отправляется.**
...
Все флаги: `djaploy --help`. Язык: `--lang en` или `DJAPLOY_LANG=en`.
